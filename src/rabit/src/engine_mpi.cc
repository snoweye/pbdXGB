 ...
 *   The solution below is one way to block error messages because
 *   MPI_Op_free() (i.e. op->Free()) and MPI_Type_free() (i.e. dtype->Free())
 *   are not allowed after MPI_Finalize().
 *   Note: The better solution is to call the destructor for
 *         each reduce handler individually who calls Init().
 *   Wei-Chen Chen 2019
 */
ReduceHandle::~ReduceHandle(void) {
  if (MPI::Is_finalized()) {
    ...
    //WCC: I doubt that this part was ever called before MPI_Finalize().
    if (handle_ != NULL) {
      MPI::Op *op = reinterpret_cast<MPI::Op*>(handle_);
      op->Free();
      delete op;
