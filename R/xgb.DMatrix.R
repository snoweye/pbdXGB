...
xgb.DMatrix <- function(data, info = list(), missing = NA, silent = FALSE, ...) {
  ...
  if (typeof(data) == "character") {
    ...
    handle <- .Call("XGDMatrixCreateFromFile_R", data, as.integer(silent), PACKAGE = "pbdXGB")
  } else if (is.matrix(data)) {
    handle <- .Call("XGDMatrixCreateFromMat_R", data, missing, PACKAGE = "pbdXGB")
    cnames <- colnames(data)
  } else if (inherits(data, "dgCMatrix")) {
    handle <- .Call("XGDMatrixCreateFromCSC_R", data@p, data@i, data@x, nrow(data), PACKAGE = "pbdXGB")
    cnames <- colnames(data)
  } else {
...
dim.xgb.DMatrix <- function(x) {
  c(.Call("XGDMatrixNumRow_R", x, PACKAGE = "pbdXGB"), .Call("XGDMatrixNumCol_R", x, PACKAGE = "pbdXGB"))
}
...
getinfo.xgb.DMatrix <- function(object, name, ...) {
  ...
  if (name != "nrow"){
    ret <- .Call("XGDMatrixGetInfo_R", object, name, PACKAGE = "pbdXGB")
  } else {
    ret <- nrow(object)
  }
...
setinfo.xgb.DMatrix <- function(object, name, info, ...) {
  if (name == "label") {
    ...
    .Call("XGDMatrixSetInfo_R", object, name, as.numeric(info), PACKAGE = "pbdXGB")
    return(TRUE)
  }
  if (name == "weight") {
    if (length(info) != nrow(object))
  ...
  if (name == "group") {
    ...
    .Call("XGDMatrixSetInfo_R", object, name, as.integer(info), PACKAGE = "pbdXGB")
    return(TRUE)
  }
...
slice.xgb.DMatrix <- function(object, idxset, ...) {
  ...
  ret <- .Call("XGDMatrixSliceDMatrix_R", object, idxset, PACKAGE = "pbdXGB")
