...
predict.xgb.Booster <- function(object, newdata, missing = NA, outputmargin = FALSE, ntreelimit = NULL,
  ...
  ret <- .Call("XGBoosterPredict_R", object$handle, newdata, option[1], as.integer(ntreelimit), PACKAGE = "pbdXGB")
...
xgb.attr <- function(object, name) {
  ...
  .Call("XGBoosterGetAttr_R", handle, as.character(name[1]), PACKAGE = "pbdXGB")
}
...
`xgb.attr<-` <- function(object, name, value) {
  ...
  .Call("XGBoosterSetAttr_R", handle, as.character(name[1]), value, PACKAGE = "pbdXGB")
...
xgb.attributes <- function(object) {
  handle <- xgb.get.handle(object)
  attr_names <- .Call("XGBoosterGetAttrNames_R", handle, PACKAGE = "pbdXGB")
  if (is.null(attr_names)) return(NULL)
  res <- lapply(attr_names, function(x) {
    .Call("XGBoosterGetAttr_R", handle, x, PACKAGE = "pbdXGB")
  })
...
`xgb.attributes<-` <- function(object, value) {
  ...
  for (i in seq_along(a)) {
    .Call("XGBoosterSetAttr_R", handle, names(a[i]), a[[i]], PACKAGE = "pbdXGB")
  }
  if (is(object, 'xgb.Booster') && !is.null(object$raw)) {
    object$raw <- xgb.save.raw(object$handle)
  }
...
`xgb.parameters<-` <- function(object, value) {
  ...
  for (i in seq_along(p)) {
    .Call("XGBoosterSetParam_R", handle, names(p[i]), p[[i]], PACKAGE = "pbdXGB")
  }
  if (is(object, 'xgb.Booster') && !is.null(object$raw)) {
    object$raw <- xgb.save.raw(object$handle)
  }
