...
xgb.iter.update <- function(booster_handle, dtrain, iter, obj = NULL) {
  ...
  if (is.null(obj)) {
    .Call("XGBoosterUpdateOneIter_R", booster_handle, as.integer(iter), dtrain, PACKAGE = "pbdXGB")
  } else {
    pred <- predict(booster_handle, dtrain)
    gpair <- obj(pred, dtrain)
    .Call("XGBoosterBoostOneIter_R", booster_handle, dtrain, gpair$grad, gpair$hess, PACKAGE = "pbdXGB")
  }
  return(TRUE)
}
...
xgb.iter.eval <- function(booster_handle, watchlist, iter, feval = NULL) {
  ...
  if (is.null(feval)) {
    msg <- .Call("XGBoosterEvalOneIter_R", booster_handle, as.integer(iter), watchlist, as.list(evnames), PACKAGE = "pbdXGB")
